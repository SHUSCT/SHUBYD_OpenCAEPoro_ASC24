...
#pragma GCC unroll 3  // jamesnull
    for (USI j = 0; j < 3; j++) {
        ...
        BYD_temp = flux_vj[j] * bvs.xi[uId_np_j]; // ctrl
        // jamesnulliu >>>>>>
        // for (USI i = 0; i < nc; i++) {
        //     // ctrl  //flux_ni[i] += flux_vj[j] * bvs.xi[uId_np_j] * bvs.xij[uId_np_j * nc + i];
        //     flux_ni[i] += BYD_temp * bvs.xij[uId_np_j * nc + i];
        // }
        // jamesnulliu >>>>>>
        flux_ni[0] += BYD_temp * bvs.xij[uId_np_j * 3];
        flux_ni[1] += BYD_temp * bvs.xij[uId_np_j * 3 + 1];
        flux_ni[2] += BYD_temp * bvs.xij[uId_np_j * 3 + 2];
...
#pragma omp parallel for private(bId_np_j, eId_np_j, uId_np_j, dId_np_j, phaseExistDj, rhoWghtU, rhoWghtD, dP, transJ, transIJ, kr, mu, xi, xij, xiP, muP, rhox, xix, mux, tmp, dFdXpU, dFdXpD, dFdXsU, dFdXsD) schedule(dynamic, 1)
    for (USI j = 0; j < np; j++) {
        ...
        for (USI i = 0; i < nc; i++) {
            ...
            dFdXpU[(i + 1) * ncol] += (tmp - transIJ * rhoWghtU * bvs.rhoP[uId_np_j] * dGamma);
            dFdXpD[(i + 1) * ncol] += -transIJ * rhoWghtD * bvs.rhoP[dId_np_j] * dGamma;

            // dS
            for (USI k = 0; k < np; k++) {
                dFdXsB[(i + 1) * ncol2 + k] += transIJ * bvs.dPcdS[bId_np_j * np + k];
                dFdXsE[(i + 1) * ncol2 + k] -= transIJ * bvs.dPcdS[eId_np_j * np + k];
                dFdXsU[(i + 1) * ncol2 + k] += Akd * bvs.dKrdS[uId_np_j * np + k] / mu * xi * xij * dP;
            }
            // dxij
            for (USI k = 0; k < nc; k++) {
                ...
                dFdXsD[(i + 1) * ncol2 + np + j * nc + k] += -transIJ * rhoWghtD * bvs.rhox[dId_np_j * nc + k] * dGamma;
            }
...
void OCPFlux01::AssembleMatAIM(const BulkConnPair& bp, const OCP_USI& c, const BulkConnVarSet& bcvs, const Bulk& bk) {
    ...
    if (bk.bulkTypeAIM.IfFIMbulk(bId))
        bIdFIM = OCP_TRUE;
    if (bk.bulkTypeAIM.IfFIMbulk(eId))
        eIdFIM = OCP_TRUE;
...
void OCPFlux02::CalFlux(const BulkConnPair& bp, const Bulk& bk) {
    ...
    for (USI j = 0; j < np; j++) {
        ...
        if (j == 0) {
            dP = (bvs.Pj[bId_np_j] - bvs.Pj[eId_np_j]) - bvs.dzMtrx[bId] * (bvs.S[bId_np_j + 1] - bvs.S[eId_np_j + 1]) * (bvs.rho[bId_np_j + 1] - bvs.rho[bId_np_j]) * GRAVITY_FACTOR * 0.5;
        } else if (j == 1) {
            dP = (bvs.Pj[bId_np_j] - bvs.Pj[eId_np_j]) + bvs.dzMtrx[bId] * (bvs.S[bId_np_j] - bvs.S[eId_np_j]) * (bvs.rho[bId_np_j] - bvs.rho[bId_np_j - 1]) * GRAVITY_FACTOR * 0.5;
        } else {
